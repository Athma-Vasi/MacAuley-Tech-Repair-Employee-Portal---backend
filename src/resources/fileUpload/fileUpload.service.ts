...
async function getQueriedTotalFileUploadsService({
	...
	try {
		const totalFileUploads = await FileUploadModel.countDocuments(filter)
			.lean()
			.exec();
...
async function getFileUploadByIdService(
	...
	try {
		const fileUpload = await FileUploadModel.findById(fileUploadId)
			.lean()
			.exec();
		return fileUpload;
	} catch (error: any) {
...
async function insertAssociatedResourceDocumentIdService(
	...
	try {
		...
		throw new Error(error, {
			cause: "insertAssociatedResourceDocumentIdService",
		});
	}
}
