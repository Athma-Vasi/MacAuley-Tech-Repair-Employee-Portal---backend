...
async function createNewMouseService(
	mouseSchema: MouseSchema,
): Promise<MouseDocument> {
	try {
...
async function getQueriedMousesService({
	...
	try {
		const mouses = await MouseModel.find(filter, projection, options)
			.lean()
			.exec();
		return mouses;
	} catch (error: any) {
...
> {
	try {
		const mouses = await MouseModel.find({})
