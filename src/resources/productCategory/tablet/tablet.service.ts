...
async function getQueriedTabletsService({
	...
	try {
		const tablets = await TabletModel.find(filter, projection, options)
			.lean()
			.exec();
		return tablets;
	} catch (error: any) {
...
async function returnAllTabletsUploadedFileIdsService(): Promise<
	Types.ObjectId[]
> {
	try {
		const tablets = await TabletModel.find({})
			...
			.lean()
			.exec();
		const uploadedFileIds = tablets.flatMap(
			(tablet) => tablet.uploadedFilesIds,
		);
