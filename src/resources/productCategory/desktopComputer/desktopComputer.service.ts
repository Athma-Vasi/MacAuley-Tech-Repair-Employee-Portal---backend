...
async function createNewDesktopComputerService(
  ...
  try {
    const newDesktopComputer = await DesktopComputerModel.create(desktopComputerSchema);
    return newDesktopComputer;
  } catch (error: any) {
    throw new createHttpError.InternalServerError("createNewDesktopComputerService");
  }
}
...
async function getQueriedDesktopComputersService({
  ...
  try {
    const desktopComputers = await DesktopComputerModel.find(filter, projection, options)
      .lean()
      .exec();
    return desktopComputers;
  } catch (error: any) {
    throw new createHttpError.InternalServerError("getQueriedDesktopComputersService");
  }
}
...
async function getQueriedTotalDesktopComputersService({
  ...
  try {
    const totalDesktopComputers = await DesktopComputerModel.countDocuments(filter)
      .lean()
      .exec();
...
async function getDesktopComputerByIdService(
  ...
  try {
    const desktopComputer = await DesktopComputerModel.findById(desktopComputerId)

      .lean()
      .exec();
    return desktopComputer;
  } catch (error: any) {
    throw new createHttpError.InternalServerError("getDesktopComputerByIdService");
  }
}
...
async function updateDesktopComputerByIdService({
  ...
  try {
    ...
    throw new createHttpError.InternalServerError("updateDesktopComputerByIdService");
  }
}

async function deleteAllDesktopComputersService(): Promise<DeleteResult> {
  try {
    const desktopComputers = await DesktopComputerModel.deleteMany({}).lean().exec();
    return desktopComputers;
  } catch (error: any) {
    throw new createHttpError.InternalServerError("deleteAllDesktopComputersService");
  }
}
...
async function deleteADesktopComputerService(
  ...
  try {
    ...
    throw new createHttpError.InternalServerError("deleteADesktopComputerService");
  }
}
