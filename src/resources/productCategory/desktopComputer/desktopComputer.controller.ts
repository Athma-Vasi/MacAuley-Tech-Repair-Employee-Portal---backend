...
const createNewDesktopComputerBulkHandler = expressAsyncHandler(
	async (
		...
		if (
			successfullyCreatedDesktopComputers.length ===
			desktopComputerSchemas.length
		) {
			response.status(201).json({
...
const deleteADesktopComputerHandler = expressAsyncHandler(
	async (
		...
		const deleteDesktopComputerResult: DeleteResult =
			await deleteADesktopComputerService(desktopComputerId);
		...
		response
			.status(200)
			.json({ message: "Desktop Computer deleted", resourceData: [] });
	},
);
