...
async function getQueriedTotalProductReviewsService({
  ...
  try {
    const totalProductReviews = await ProductReviewModel.countDocuments(filter).lean().exec();
...
async function deleteAProductReviewService(productReviewId: string): Promise<DeleteResult> {
  try {
    const productReview = await ProductReviewModel.deleteOne({ _id: productReviewId })
      .lean()
      .exec();
    return productReview;
  } catch (error: any) {
