  ...
  async (
    ...
    const fileUploadsArrArr = await Promise.all(
      headphones.map(async (headphone) => {
        const fileUploadPromises = headphone.uploadedFilesIds.map(async (uploadedFileId) => {
...
const updateHeadphoneByIdHandler = expressAsyncHandler(
  async (
    ...
    if (!headphoneExists) {
      response.status(404).json({ message: 'Headphone does not exist', resourceData: [] });
      return;
    }
...
const deleteAHeadphoneHandler = expressAsyncHandler(
  async (
    ...
    const deleteHeadphoneResult: DeleteResult = await deleteAHeadphoneService(headphoneId);
