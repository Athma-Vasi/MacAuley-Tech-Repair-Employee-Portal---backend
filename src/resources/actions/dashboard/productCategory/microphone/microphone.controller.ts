...
const createNewMicrophoneBulkHandler = expressAsyncHandler(
  async (
    ...
    const successfullyCreatedMicrophones = newMicrophones.filter((microphone) => microphone);
  ...
  async (
    ...
    const fileUploadsArrArr = await Promise.all(
      microphones.map(async (microphone) => {
        const fileUploadPromises = microphone.uploadedFilesIds.map(async (uploadedFileId) => {
...
const updateMicrophoneByIdHandler = expressAsyncHandler(
  async (
    ...
    if (!microphoneExists) {
      response.status(404).json({ message: 'Microphone does not exist', resourceData: [] });
      return;
    }
...
const deleteAMicrophoneHandler = expressAsyncHandler(
  async (
    ...
    const deleteMicrophoneResult: DeleteResult = await deleteAMicrophoneService(microphoneId);
