...
import { ComputerCaseDocument, ComputerCaseModel, ComputerCaseSchema } from './computerCase.model';
...
async function getQueriedComputerCasesService({
  ...
  try {
    const computerCases = await ComputerCaseModel.find(filter, projection, options).lean().exec();
...
async function getQueriedTotalComputerCasesService({
  ...
  try {
    const totalComputerCase = await ComputerCaseModel.countDocuments(filter).lean().exec();
...
async function returnAllComputerCasesUploadedFileIdsService(): Promise<Types.ObjectId[]> {
  try {
    const computerCases = await ComputerCaseModel.find({}).select('uploadedFilesIds').lean().exec();
    const uploadedFileIds = computerCases.flatMap((computerCase) => computerCase.uploadedFilesIds);
    return uploadedFileIds;
  } catch (error: any) {
    throw new Error(error, { cause: 'returnAllComputerCasesUploadedFileIdsService' });
  }
}
...
async function deleteAComputerCaseService(
  ...
  try {
    const computerCase = await ComputerCaseModel.deleteOne({ _id: computerCaseId }).lean().exec();
    return computerCase;
  } catch (error: any) {
