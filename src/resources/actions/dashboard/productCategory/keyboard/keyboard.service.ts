...
async function createNewKeyboardService(keyboardSchema: KeyboardSchema): Promise<KeyboardDocument> {
  try {
...
async function getQueriedTotalKeyboardsService({
  ...
  try {
    const totalKeyboard = await KeyboardModel.countDocuments(filter).lean().exec();
...
async function getKeyboardByIdService(
  ...
  try {
    const keyboard = await KeyboardModel.findById(keyboardId).select('-__v').lean().exec();
    return keyboard;
  } catch (error: any) {
...
async function updateKeyboardByIdService({
  ...
  keyboardId: Types.ObjectId | string;
  fieldsToUpdate: Record<keyof KeyboardDocument, KeyboardDocument[keyof KeyboardDocument]>;
}): DatabaseResponseNullable<KeyboardDocument> {
  try {
...
async function returnAllKeyboardsUploadedFileIdsService(): Promise<Types.ObjectId[]> {
  try {
    const keyboards = await KeyboardModel.find({}).select('uploadedFilesIds').lean().exec();
    const uploadedFileIds = keyboards.flatMap((keyboard) => keyboard.uploadedFilesIds);
    return uploadedFileIds;
  } catch (error: any) {
    throw new Error(error, { cause: 'returnAllKeyboardsUploadedFileIdsService' });
  }
}

async function deleteAKeyboardService(keyboardId: Types.ObjectId | string): Promise<DeleteResult> {
  try {
    const keyboard = await KeyboardModel.deleteOne({ _id: keyboardId }).lean().exec();
    return keyboard;
  } catch (error: any) {
