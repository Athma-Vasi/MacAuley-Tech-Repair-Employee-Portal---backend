...
async function getQueriedTotalDesktopComputersService({
  ...
  try {
    const totalDesktopComputer = await DesktopComputerModel.countDocuments(filter).lean().exec();
...
async function returnAllDesktopComputersUploadedFileIdsService(): Promise<Types.ObjectId[]> {
  try {
    ...
    throw new Error(error, { cause: 'returnAllDesktopComputersUploadedFileIdsService' });
  }
}
...
async function deleteADesktopComputerService(
  ...
  try {
    const desktopComputer = await DesktopComputerModel.deleteOne({ _id: desktopComputerId })
      .lean()
      .exec();
    return desktopComputer;
  } catch (error: any) {
