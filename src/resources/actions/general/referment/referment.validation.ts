...
const createRefermentJoiSchema = Joi.object({
  ...
  candidateCurrentJobTitle: Joi.string().regex(GRAMMAR_TEXT_INPUT_REGEX).required(),
  candidateCurrentCompany: Joi.string().regex(GRAMMAR_TEXT_INPUT_REGEX).required(),
  ...
  positionJobDescription: Joi.string().regex(GRAMMAR_TEXTAREA_INPUT_REGEX).required(),
